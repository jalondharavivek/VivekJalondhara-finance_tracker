...
import { transactondataadd ,edittransactiondata} from "../../store/slice/transactionsl";
...
let userSchema = yup.object().shape({
  ...
  toaccount: yup
    ...
    .notOneOf([yup.ref("fromaccount")], "select diffrent to and from account"),
  ...
  receipt: yup
    ...
    .test("type", "We only support jpeg and jpg format", function (value: any) {
  ...
  notes: yup
    ...
    .min(2, "Notes Min 2 Characters")
    .max(225, "Notes Max 225 Character"),
  // .string("").trim()
...
const Transactionadd = (props : any) => {
  console.log(props.all,"prop");
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<transactionadddata>({ resolver: yupResolver(userSchema) });

  const onSubmitHandler = async (data: transactionadddata) => {
    if (typeof data.receipt !== "string") {
      const url: string = (await bs(data.receipt[0])) as string;
      data.receipt = url;
      console.log(data.receipt);

if(props?.all?.id){
var ediittradata  = {...data};

ediittradata  = { ...data, id: props?.all?.id };
     
        dispatch(edittransactiondata({id:props?.all?.id,data:ediittradata}))
navigate("/")
}else{


      let tradata = { ...data };
      const tralength = transactiondata.reduce((addtra: any) => addtra + 1, 0);
      const idtransaction = transactiondata[tralength - 1].id;
      tradata.id = idtransaction + 1;
      // console.log(url,"url");
      dispatch(transactondataadd(tradata));
      navigate("/");
}
    }
  };
